...
# Let's just make another function dedicated only for the drl application.
# Let's use train_net_fedcon as a baseline
    ...
    if args.alg == 'moon':
        ...
        if args.load_pool_file:
            for nets_id in range(args.model_buffer_size):
                old_nets, _, _ = init_nets(args.net_config, args.n_parties, args, device='cuda:0')
        ...
        for round in range(n_comm_rounds):
            # It performs global model averaging. The local DRL needs to be done in local_train function
            # arg.epoch specifies how many number of training needs to be done in one local training instance.
            # everything related to the DRL training needs to be done in local training. 
            # However, it is better to initialise the env here. 
            ...
            # This section needs to be experimented for different network strategies for averaging.
            ...
            if len(old_nets_pool) < args.model_buffer_size:
                logger.info(f'The length of old_nets_pool: {old_nets_pool}, args.model_buffer_size: {args.model_buffer_size}')
                old_nets = copy.deepcopy(nets)
                for _, net in old_nets.items():
                    net.eval()
                    for param in net.parameters():
        ...
        nets, _, _ = init_nets(args.net_config, 1, args, device='cuda:0')
        # nets[0].to(device)
